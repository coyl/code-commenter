...
func (a *Adapter) UploadJob(ctx context.Context, jobID, prompt, rawJSON, fullCode, fullCodePlain string, segments []ports.JobSegment, segmentAudio [][]byte) error {
	if a.Store == nil {
		return nil
	}
	...
	return a.Store.UploadJob(ctx, jobID, prompt, rawJSON, fullCode, fullCodePlain, stored, segmentAudio)
}
...
func (NoopAdapter) UploadJob(context.Context, string, string, string, string, string, []ports.JobSegment, [][]byte) error {
	return nil
}
